    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'input[name="layTableCheckbox"]+', function () {
            var checkbox = $(this).prev()
                ...
                , checked = checkbox[0].checked
                , isAll = checkbox.attr('lay-filter') === 'layTableAllChoose'
                , thisData = table.cache, treeId = that.config.treeId
                , treeUpId = that.config.treeUpId;

            //先将数据分类
            /*
              map={
                 treeUpId:[child1,child2]
              }
            */
            var childMapData = {};
            ...
            for (var i in thisData) {
                var key = thisData[i];
                if (childMapData.hasOwnProperty(key)) {
                    childMapData[key].push(thisData[i]);
                } else {
                    var child = [];
                    child.push(thisData[i]);
                    childMapData[key] = child;
                }
            }
            ...
            if (isAll) {
                ...
                that.setCheckData(index, checked);
                //获取本级id,递归下级全选
                var checkId = thisData[index];
                downCheck(childMapData, checkId, checked);


                //检测平级是否都选中，如果都选中，则设置父级选中，否则父级不选中
                var checkUpId = thisData[index];//获取父级id
                upCheck(childMapData, checkUpId);
            ...
            layui.event.call(this, MOD_NAME, 'checkbox(' + filter + ')', {
                checked: checked
                , data: table.cache ? (table.cache[index] || {}) : {}
                , type: isAll ? 'all' : 'one'
            });

            //递归选中下级树
            function downCheck(childMapData, checkId, checked) {
                var child = childMapData[checkId];
                //该级下的子树全选
                for (var i in child) {
                    var data = child[i];
                    childs[data.LAY_TABLE_INDEX].checked = checked;
                    that.setCheckData(data.LAY_TABLE_INDEX, checked);
                    //id
                    var id = data;
                    downCheck(childMapData, id, checked);
                }
            }

            //递归检测上级树
            function upCheck(childMapData, checkUpId) {
                //获取平级的所有数据
                var levelArray = childMapData[checkUpId];
                var allCheckFlag = true;
                for (var i in levelArray) {
                    //存在没选中的
                    if (levelArray[i].LAY_CHECKED == undefined || !levelArray[i].LAY_CHECKED) {
                        allCheckFlag = false;
                        break;
                    }
                }
                var parent = findParent(thisData, checkUpId);
                if (parent != undefined) {
                    childs
                        .checked = allCheckFlag;
                    that.setCheckData(parent.LAY_TABLE_INDEX, allCheckFlag);
                    //递归
                    var upId = parent;
                    upCheck(childMapData, upId);
                }
            }

            function findParent(thisData, parentId) {
                for (var i in thisData) {
                    if (thisData[i] == parentId) {
                        return thisData[i]
                    }
                }
            }
