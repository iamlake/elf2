...
public class ClassUtil {
	 ...
	 * @return
	 */
	public List<Class<?>> getAllClassByPackageName(Package pkg) {
	 ...
	 * @return
	 */
	public List<Class<?>> getAllClassByPackageName(String packageName) {
		hasWildCard = packageName.contains("*");		
		if(hasWildCard){
			String parentPkg = packageName.substring(0, packageName.indexOf("*")-1);
			this.regEx = packageName.replaceAll("//*", "[a-zA-Z_]{1,}[0-9]{0,}");
			return getClasses(parentPkg);
		}else{
			// 获取当前包下以及子包下所以的类
